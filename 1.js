...
const CONFIG = {
  ...
  MAX_HISTORY: 100,
  PATTERN_LENGTH: 14,
  XENG16_API: 'https://taixiumd5.backend-98423498294223x1.online/api/md5luckydice/GetSoiCau'
};
...
async function getLatestResult() {
  try {
    ...
    // Tìm phiên có SessionId lớn nhất
    const latestRecord = dataArray.reduce((prev, current) => {
      return (parseInt(prev.SessionId) > parseInt(current.SessionId)) ? prev : current;
    });
    
    // Kiểm tra dữ liệu hợp lệ
    if (!latestRecord.SessionId || !latestRecord.FirstDice || !latestRecord.SecondDice || !latestRecord.ThirdDice) {
      return null;
    }
    
    const result = {
      SessionId: parseInt(latestRecord.SessionId),
      FirstDice: parseInt(latestRecord.FirstDice),
      SecondDice: parseInt(latestRecord.SecondDice),
      ThirdDice: parseInt(latestRecord.ThirdDice),
      DiceSum: parseInt(latestRecord.DiceSum) || 0,
      BetSide: parseInt(latestRecord.BetSide),
